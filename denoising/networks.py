...
class WideModel(nn.Module):
    def __init__(self, para):
        ...
        self.block = nn.Sequential(
            modules.conv1x1(4, para.n_feats),
            ...
            modules.conv5x5(para.n_feats, 3, stride=1)
        )
...
class DparNet(nn.Module):
    def __init__(self, para):
        ...
        self.merge = modules.conv1x1(6, 3)
...
class PPN(nn.Module):
    ...
    def forward(self, input_data):
        f = self.CNN1(input_data.permute(0, 2, 1, 3, 4))
...
if __name__ == '__main__':
    ...
    import numpy as np
    para = Parameter().args
    model = DparNet(para).cuda()
    input = torch.randn(1, 15, 3, 256, 256).cuda()  # frame length: 15 or 7
    input2 = torch.randn(1, 1, 256, 256).cuda()
    parameters = sum([np.prod(list(p.size())) for p in model.parameters()])
    print('model_params: {:4f}M'.format(parameters / 1000 / 1000))
    flops, params = profile(model, inputs=(input,input2,))
    print("FLOPs=", str(flops / 11 / 1e10))

    start_time = time.time()
    for i in range(100):
        out = model(input, input2)
    end_time = time.time()
    print('time:{:.4f}s'.format((end_time - start_time) / 100 / 11))
