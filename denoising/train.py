...
if __name__ == '__main__':
    ...
    # Dataset
    train_dataset = dataset.DeNoisingDataset(para, 'train/')
    ...
    valid_dataset = dataset.DeNoisingDataset(para, 'valid/')
    ...
    lr = 1e-4
    opt = optim.Adam(DparNet.parameters(), lr)
    ...
    model_path = para.save_dir + 'IpairNet_tiny_data/'
    ...
    for epoch in range(1, 101):
        start = time.time()
        a = utils.train(device, train_loader, DparNet, PPN, opt, criterion, perception, metrics, epoch, para.batch_size)
        b = utils.valid(device, valid_loader, DparNet, PPN, metrics)
        train_PSNR.append(a), valid_PSNR.append(b)
        end = time.time()

        print('time:{:.2f}s'.format(end - start))
        checkpoint = {'model': DparNet.state_dict()}
